...
import { 
  ...
  ChevronLeft, ChevronRight, Info, Clock, Check, X, Search, UserPlus, AlertTriangle,
  Utensils, Car, Ticket, Home as HomeIcon, MoreHorizontal
} from 'lucide-react';
...
const CATEGORY_ICONS: Record<ExpenseCategory, React.ReactNode> = {
  ...
  [ExpenseCategory.ACCOMMODATION]: <HomeIcon size={16} />,
...
export default function TripDetails({ params }: { params: { id: string } }) {
  ...
  // [PULL POINT] Getting fresh trip and notification state from global store
  ...
  // [PULL POINT] Reactive filtering of cloud notifications for status button
  const tripInvitations = useMemo(() => {
    if (!trip) return [];
    // Strict string comparison ensures reliability across cloud/local IDs
  ...
  /**
   * [PUSH POINT] Expense saved triggers global state update and eventual Turso Sync
   */
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-black transition-colors duration-200">
      ...
      <div className="shrink-0 z-20 bg-white dark:bg-black px-4 py-3 flex justify-between items-center transition-colors border-b border-neutral-100 dark:border-neutral-900">
        ...
        <h1 className="font-bold text-lg text-neutral-900 dark:text-white truncate max-w-[200px] tracking-tight">{trip.name}</h1>
        {trip.status === 'ongoing' ? (
             <button 
             onClick={() => setIsEndModalOpen(true)}
             className="text-red-500 font-bold text-sm px-3 py-1 bg-red-50 dark:bg-red-900/10 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/20"
           >
             End
           </button>
        ) : <div className="w-8" />}
        ...
        <div className="p-4 pb-28">
            ...
            <div className="bg-brand-gradient rounded-[2rem] p-6 text-white shadow-xl shadow-orange-500/20 dark:shadow-none mb-8 relative overflow-hidden">
                <div className="relative z-10">
                    <div className="text-white/80 text-sm font-bold mb-1">Total Spending</div>
            ...
            <div className="mb-6">
                ...
                <div className="relative">
                    {canScrollLeft && (
                        <button type="button" onClick={() => scrollMembers('left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-40 p-2 bg-white/90 dark:bg-black/90 backdrop-blur-sm rounded-r-xl shadow-lg text-neutral-600 dark:text-neutral-300">
                            <ChevronLeft size={20} />
                        </button>
                    )}
                    ...
                    >
                        {trip.status === 'ongoing' && (
                            <div className="flex gap-2 shrink-0 relative z-30 pointer-events-auto">
                                <button 
                                    type="button" 
                        ...
                        {trip.members.map((member) => (
                            ...
                            >
                                ...
                                <span className="text-[10px] font-bold text-neutral-600 dark:text-neutral-400 text-center truncate w-full px-1">
                    ...
                    {canScrollRight && (
                        <button type="button" onClick={() => scrollMembers('right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-40 p-2 bg-white/90 dark:bg-black/90 backdrop-blur-sm rounded-l-xl shadow-lg text-neutral-600 dark:text-neutral-300">
            ...
            <div className="flex justify-between items-center mb-4 px-2">
                ...
                {trip.status === 'ongoing' && <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Shared Event</span>}
            </div>
            
            <div className="space-y-3">
                {trip.expenses.length === 0 && (
                    <div className="text-center py-12 text-neutral-400 dark:text-neutral-500 text-sm italic font-medium">
                        No transactions yet.
                    </div>
                )}
                {[...trip.expenses].reverse().map(expense => (
                    ...
                    >
                        ...
                        >
                            ...
                            {trip.status === 'ongoing' && (
                                <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-20">
                                    <button
                                        type="button"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        className="p-2.5 bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/60 transition-all hover:scale-110 active:scale-95 shadow-sm"
                                    >
                                        <Trash2 size={20} />
                                    </button>
                                </div>
      ...
      <Modal isOpen={isEndModalOpen} onClose={() => setIsEndModalOpen(false)} title="End Event?">
        <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-sm text-neutral-600 dark:text-neutral-300">
                Are you sure you want to end <strong>{trip.name}</strong>? You won't be able to add more expenses, and final balances will be saved in history.
            </p>
        </div>
      ...
      {/* Debt Modal */}
      <Modal isOpen={isDebtModalOpen} onClose={() => setIsDebtModalOpen(false)} title="Who Owes Who">
        <div className="space-y-4">
          <div className="flex items-center justify-between pb-2 mb-2 border-b border-neutral-100 dark:border-neutral-800">
                ...
                <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold ${showMySplitsOnly ? 'text-brand-pink' : 'text-neutral-400'}`}>My Splits</span>
                    <button 
                        type="button"
          ...
          {displayedDebts.length === 0 ? (
            <div className="text-center text-neutral-500 dark:text-neutral-400 py-8 font-medium italic">All balances are settled!</div>
          ) : (
      ...
      {/* Expense Modal */}
      <Modal isOpen={isExpenseModalOpen} onClose={() => setIsExpenseModalOpen(false)} title={editingExpense ? "Modify Expense" : "New Expense"}>
        <form onSubmit={handleSaveExpense} className="space-y-5">
            <div className="relative flex items-center justify-center py-4">
                ...
                <input 
                    ...
                    className="w-full text-center py-2 bg-transparent border-b-2 border-neutral-100 dark:border-neutral-800 focus:border-brand-pink font-bold text-4xl text-neutral-900 dark:text-white outline-none"
                    placeholder="0.00"
                />
            </div>
            <input 
                type="text" value={description} onChange={e => setDescription(e.target.value)}
                className="w-full px-4 py-4 bg-neutral-50 dark:bg-neutral-900 rounded-2xl border-none focus:ring-2 focus:ring-brand-pink text-neutral-900 dark:text-white"
                ...
                <div className="flex flex-wrap gap-2">
                    {Object.values(ExpenseCategory).map(c => (
                        <button
                            ...
                            className={`px-3 py-2 rounded-xl text-sm font-bold border ${category === c ? 'bg-neutral-900 dark:bg-white text-white dark:text-black border-neutral-900 dark:border-white shadow-md' : 'bg-neutral-100 dark:bg-neutral-900 text-neutral-500 border-transparent'}`}
                        >
                            {c}
                        </button>
                    ))}
      ...
      <Modal isOpen={!!friendToInvite} onClose={() => setFriendToInvite(null)} title="Invite Friend?">
      ...
      <Modal isOpen={isAddMemberModalOpen} onClose={() => setIsAddMemberModalOpen(false)} title="Add Local Guest">
