/**
 * The AddTrip page allows you to start a new event.
 * You can name the event, add guest members manually, 
...
const AddTrip: React.FC = () => {
  ...
  // Names of local members who don't have accounts
  const [members, setMembers] = useState<string[]>([]);
  // Usernames of friends we are inviting from Turso
  ...
  // Auto-add you to the list so you are part of the split
  useEffect(() => {
  ...
  const toggleFriend = (friendName: string) => {
    if (invitedFriends.includes(friendName)) {
      ...
      // To ensure a user isn't both a guest and an invited friend
  ...
  /**
   * Final step: Create the event locally and send invitations into Turso.
   */
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (tripName && members.length > 0 && !isCreating) {
      ...
      try {
        ...
        const createdTrip = await addTrip(tripName, members);
        
        // 2. Send invitations to friends
        for (const friend of invitedFriends) {
          try {
            await inviteFriendToTrip(friend, createdTrip.id, tripName);
          } catch (err) {
  ...
  return (
    <div className="p-6 min-h-full bg-white dark:bg-black transition-colors duration-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 pb-32">
        ...
        {/* Co-Owners / Cloud Friends Section */}
        <div className="space-y-3">
          ...
          <p className="text-[10px] text-neutral-400 font-medium px-1">Friends will be added to the split once they accept your invitation.</p>
        </div>

        {/* Immediate Members / Guests Section */}
        <div className="space-y-3">
          ...
          {!isSelfAdded && (
            <button
                ...
                className="flex items-center gap-3 w-full p-4 rounded-2xl bg-brand-pink/5 border border-brand-pink/20 text-brand-pink hover:bg-brand-pink/10 transition-colors text-left group active:scale-[0.98] animate-in slide-in-from-left-2 shadow-sm"
            >
                ...
                <div className="flex-1">
                    ...
                    <div className="text-[10px] opacity-70">Add you as an immediate member</div>
                </div>
          ...
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Name of guest member..."
              value={currentMember}
            ...
            <button
              ...
              className="bg-neutral-900 dark:bg-white text-white dark:text-black w-14 rounded-2xl disabled:opacity-50 hover:opacity-90 active:scale-95 transition-all flex items-center justify-center shadow-lg relative z-10"
            >
              <Plus size={24} />
            </button>
          </div>
        ...
        <div className="pt-10 flex flex-col gap-4">
             <Button 
                ...
                className="flex items-center justify-center gap-3 py-5 text-lg shadow-xl hover:scale-[1.02] transition-transform"
             >
                {isCreating ? <RefreshCw size={22} className="animate-spin" /> : <Sparkles size={22} />}
                {isCreating ? 'Finalizing...' : 'Create Event'}
             </Button>
        </div>
      </form>
