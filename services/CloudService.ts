/**
 * This is the "Delivery Man" service. 
 * Its job is to take data from your app and physically place it into the Turso Database (PUSH),
 * or go to the Turso Database and bring data back to your app (PULL).
 */
import { AppData, Trip, Expense, Friend, Notification } from '../types';
import { db } from './db';

class CloudService {
  /**
   * [PUSH POINT - SETUP]
   * This builds the "shelves" in the database. It tells Turso how to store our data
   * (tables for Users, Trips, Expenses, etc.)
   */
  async initSchema() {
    try {
      await db.batch([
  ...
  /**
   * [PUSH/PULL POINT - AUTH]
   * SignUp: PUSHES a new user row to Turso.
   * Login: PULLS a user row to check if the password matches.
   */
  ...
  /**
   * [PUSH POINT] Updates the password shelf in Turso.
   */
  ...
  /**
   * [PULL POINT - MASTER PULL]
   * This is a giant "shopping list" that grabs every single piece of information 
   * belonging to a user (Trips, Expenses, Friends) from Turso in one go.
   */
  async fetchUserData(username: string): Promise<AppData | null> {
    const cleanUsername = username.trim();
    
    try {
      // Pull user profile and list of trips they own
      ...
      // Pull the detailed data for every trip ID found
      ...
      // Organising the flat Turso data into nice Trip objects
  ...
  /**
   * Helper to pull secondary items like friends and pending notifications.
   */
  ...
  /**
   * [PUSH POINT - ACTION]
   * Updates Turso with your response to a notification and handles the logic 
   * for adding someone to a trip or a friend list.
   */
  async respondToNotification(notificationId: string, status: 'ACCEPTED' | 'DECLINED'): Promise<void> {
    ...
    if (status === 'ACCEPTED') {
      if (type === 'FRIEND_REQUEST') {
        ...
        // 2. Add to members list in the trip row
  ...
  /**
   * [PUSH POINT - MASTER PUSH]
   * This is the heavy lifter. It takes all the current data in your app 
   * and UPSERTS (Inserts if new, Updates if exists) it into the Turso Database.
   */
  async syncToCloud(data: AppData): Promise<boolean> {
    ...
    try {
      ...
      // Push every trip and its expenses
      ...
      // Execute all PUSHES in one efficient batch
  ...
  /**
   * [PUSH POINT] Deletes a user from the participant table in Turso.
   */
  ...
  /**
   * [PUSH POINT] Removes all data related to a specific trip from Turso.
   */
  ...
  /**
   * [PUSH POINT] Clears ended trips for a specific user in Turso.
   */
  ...
  /**
   * [PULL POINT] Asks Turso for a list of users matching a search term.
   * Can search by username or phone number.
   */
  ...
  /**
   * [PUSH POINT] Creates a friend request notification in Turso.
   */
  ...
  /**
   * [PUSH POINT] Creates a trip invite notification in Turso.
   */
  ...
  /**
   * [PUSH POINT] Removes the friend connection rows from Turso.
   */
